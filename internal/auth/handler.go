...
func (a *Handler) HandleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Log.Error("Something went wrong while fetching the user", "username", req.Username, "err", err)
		api.Fail(w, http.StatusInternalServerError, "Something went wrong while fetching the user", err)
		return
	}

	if u == nil {
		logger.Log.Error("Invalid credentials", "username", req.Username)
	...
	if err != nil {
		logger.Log.Error("Something went wrong while fetching the session", "user_id", u.Id, "err", err)
	...
	if ses != nil {
		if err := session.DeleteSession(a.db, ses.Id); err != nil {
			logger.Log.Error("Something went wrong while deleting the session", "user_id", u.Id, "err", err)
	...
	logger.Log.Info("Login successful", "user_id", u.Id)
...
func (a *Handler) HandleSignUp(w http.ResponseWriter, r *http.Request) {
	...
	logger.Log.Info("User created successfully", "user_id", u.Id)
...
func (a *Handler) HandleLogout(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Log.Error("Failed to decode session token", "err", err)
		api.Fail(w, http.StatusUnauthorized, "Invalid session token", nil)
		return
	}
	err = session.DeleteSession(a.db, tokenBytes)
	if err != nil {
		logger.Log.Error("Failed to delete session", "err", err)
...
func (a *Handler) HandleMe(w http.ResponseWriter, r *http.Request) {
	...
	ses, err := session.GetSession(a.db, tokenBytes)
	if err != nil {
		logger.Log.Error("Something went wrong while fetching the session", "err", err)
		api.Fail(w, http.StatusInternalServerError, "Something went wrong while fetching the session", err)
		return
	}
	if ses == nil {
		logger.Log.Error("Session not found", "id", token)
		api.Fail(w, http.StatusNotFound, "User not found", nil)
		return
	}
	u, err := user.GetUser(a.db, int(ses.UserId))
	if err != nil {
		logger.Log.Error("User not found", "user_id", ses.UserId, "err", err)
		api.Fail(w, http.StatusNotFound, "User not found", err)
		return
	}
	logger.Log.Info("User found", "user_id", u.Id)
